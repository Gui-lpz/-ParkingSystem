...
public class ParkingLotWindow extends JInternalFrame implements ActionListener {
    ...
    public JTextField txtName, txtRegSpaces, txtDisabilitySpaces, txtMotoSpaces,
            txtHeavySpaces, txtBikeSpaces, txtOtherSpaces;
    ...
    public ParkingLotWindow() {
        super("Configurar Parqueo", false, true, false, true);
    ...
    private void initComponents() {
        ...
        addLabelAndField("Espacios Autos Livianos :", txtRegSpaces = new JTextField("10"), 60);
        addLabelAndField("Espacios preferenciales:", txtDisabilitySpaces = new JTextField("2"), 95);
        ...
        btnSave.setBounds(80, 500, 150, 30);
        btnSave.addActionListener(this);

        btnCancel = new JButton("Cancelar");
        btnCancel.setBounds(260, 500, 100, 30);
    ...
    public void loadParkingLotData(ParkingLot pl) {
        parkingId = pl.getId();
        txtName.setText(pl.getName());
        btnSave.setText("Modificar Parqueo");
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            ...
            VehicleType vLiviano = new VehicleType(1, "Liviano", (byte) 4,
                    Float.parseFloat(txtFeeLiviano.getText()), "L");
            VehicleType vMoto = new VehicleType(2, "Moto", (byte) 2,
                    Float.parseFloat(txtFeeMoto.getText()), "M");
            VehicleType vPesado = new VehicleType(3, "Pesado", (byte) 6,
                    Float.parseFloat(txtFeeHeavy.getText()), "P");
            VehicleType vBici = new VehicleType(4, "Bicicleta", (byte) 2,
                    Float.parseFloat(txtFeeBike.getText()), "B");
            VehicleType vOther = new VehicleType(5, "Otro", (byte) 4,
                    Float.parseFloat(txtFeeOther.getText()), "O");
            ...
            for (int i = 0; i < cDis; i++) {
                spaces[count++] = new Space(count + 1, true, false, vLiviano);
            }
            for (int i = 0; i < cMot; i++) {
                spaces[count++] = new Space(count + 1, false, false, vMoto);
            }
            for (int i = 0; i < cPes; i++) {
                spaces[count++] = new Space(count + 1, false, false, vPesado);
            }
            for (int i = 0; i < cBic; i++) {
                spaces[count++] = new Space(count + 1, false, false, vBici);
            }
            for (int i = 0; i < cOth; i++) {
                spaces[count++] = new Space(count + 1, false, false, vOther);
            }
            for (int i = 0; i < cReg; i++) {
                spaces[count++] = new Space(count + 1, false, false, vLiviano);
            }

            if (btnSave.getText().contains("Modificar")) {
                parkingController.updateParkingLot(parkingId, txtName.getText(), spaces);
                JOptionPane.showMessageDialog(this, "Parqueo modificado con éxito.");
            } else {
                parkingController.registerParkingLot(txtName.getText(), spaces);
                JOptionPane.showMessageDialog(this, "Parqueo creado con éxito.");
            }

            dispose();

        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Error: revise los datos ingresados.");
        }
    }
}
